...
const App: React.FC = () => {
  ...
  const [debouncedSetPower] = useDebouncedCallback(
    (zone: string, on: boolean) => {
      axios.post('http://' + HostName + '/zones/' + zone + '/pr', on ? '01' : '00')
  ...
  const [debouncedSetVolume] = useDebouncedCallback(
    (zone: string, volume: SliderValue) => {
      axios.post('http://' + HostName + '/zones/' + zone + '/vo', pad(String(volume), 2))
        .then(debouncedRefreshZones)
        .catch(function (error) {
          console.log(error);
        });
  ...
  const getZoneStatus = () => {
    return zoneStatusList.find(z => z.zone === zoneInfo.zone) || {
      zone: '00',
      pa: '00',
      pr: '00',
      mu: '00',
  ...
  return (
    <Layout>
      ...
      >
        <div className="logo" />
        <Menu theme="dark" mode="inline" defaultSelectedKeys={['4']} onSelect={(info) => {
          if ('0' === info.key) {
          ...
          setTimeout(() => { setMenuCollapsed(true); }, 250);
        }}>
          <Menu.Item key="0">
